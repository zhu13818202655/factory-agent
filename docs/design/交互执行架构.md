...
### 4.1 护栏链与 LLM 往返（ADR-0008）

停止点之间，管线在「鉴权通过」与「业务取数」之间还有一道**越权护栏**：判定由 LLM 产出，但最终裁决与拒绝文案都在本地（`application/session/consistency.py`）。它的位置由一条不变式约束——**任何业务数据调用都排在护栏之后**（AGENTS.md 架构不变式 1）。

默认（`FACTORY_AGENT_SCOPE_GUARD_MODE=merged`）把护栏判定并入同一次 EXTRACT 调用：payload 多返回一个 `scope` 子对象，正常路径因此每次业务查询少一次 LLM 往返（`llm_call_fact` 3 → 2 行）。`scope` 键**缺失或非法**时回落一次**独立** `SCOPE_GUARD` 调用；两次都不成时仍是 fail-open（warning + 继续执行）。

合并只改变「在哪里问」，不改变「怎么判」：判定规则逐字共享，合并判定同样**只能拒绝、不能放宽**，未覆盖到的语义一律按「没拿到判定」处理（绝不把缺失当 `within`）。计量上，合并路径不发独立 SCOPE_GUARD 事件，改在 EXTRACT 事件上带 `includes_scope: true` 与 `scope_verdict`（只存 verdict，不存目标文本）。

| 模式 | 行为 |
|---|---|
| `merged`（默认） | 判定并入 EXTRACT；缺失/非法时回落独立调用 |
| `dedicated` | 强制独立调用（改造前行为，用于回退与灰度对照） |
...
| `scope_guard_mode` | `merged` | 护栏判定并入 EXTRACT（§4.1 / ADR-0008）；置 `dedicated` 回到独立调用 |
| `session_thinking_enabled` | `true` | 推理原文（`interaction.thinking`）总开关；关闭后整轮不发该事件、也不写 `kind="thinking"` 消息 |
| `session_thinking_timeout_seconds` | 20s | 单次旁白调用上限；超时即放弃，绝不让回答变慢 |
| `session_thinking_tick_seconds` | 0.25s | 等待期内检查并冲刷分片的间隔 |
| `session_thinking_max_output_tokens` | 160 | 单次旁白调用的输出上限 |
| `llm_thinking_stream_alias` | `factory-fast` | 旁白调用的模型别名（刻意与其所描述的调用错开） |

链路优化引入的其余开关（`time_parse_mode`、`mes_fanout_concurrency`）与导出后端配置见
`docs/design/链路优化方案.md` §3.6。
