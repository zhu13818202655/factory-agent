...
flowchart TD
    ...
    Y2["导出 XLSX（内存 buffer，不留存）<br/>结果卡片 card（recipe 声明，可选）<br/>ports/card.py build_card"] --> Z3["阶段 ANSWERED『完成』<br/>result 事件（含 card）+ 终态事件 + 用量事件落库<br/>llm_call ×3 / mes_call ×N / interaction_completed"]
...
| 15 | 结果卡片与导出 | card 形态怎么声明/构建？XLSX 是否留存？ | `ports/card.py`（`CardTableSpec`、`build_card`）、`export_service.py` + `export/xlsx.py`（内存 buffer 不留存）；card 挂载点 `pipeline.py::_complete_result` | `configs/knowledge/recipes/*.yaml`（`card:` 块）、`execution/recipes.py`（`_validate_card`、`to_table_spec`） |
