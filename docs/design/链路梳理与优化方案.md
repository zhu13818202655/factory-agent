...
flowchart TD
    ...
    T --> U["按 recipe DAG 执行 API 步骤（串行）<br/>ScopedExecutor.execute_full_step → HongzhaoMesAdapter<br/>④ MES HTTP：GongziMxQuery（分页由 PagerBudget 驱动：首页 20000 探测，超过一页即升至 50000/页重走）<br/>pydantic 校验 → 行写入 DuckDB 沙箱"]
    ...
    X --> Y["⑤ LLM 调用 SUMMARIZE（factory-summary）<br/>ResultSummarizer 只喂元数据+合计（明细行不进 prompt）<br/>失败则确定性兜底文案，不影响交互"]
    Y2["导出 XLSX（内存渲染后落盘保留，默认 90 天）<br/>结果卡片 card（recipe 声明，可选）<br/>ports/card.py build_card"] --> Z3["阶段 ANSWERED『完成』<br/>result 事件（含 card）+ 终态事件 + 用量事件落库<br/>llm_call ×3 / mes_call ×N / interaction_completed"]
    Z3 --> Z4["用量计量独立事务直写 PostgreSQL（失败告警不回滚）"]
    Z1 --> Z4
```

> 时序要点：`authorize` 在 `start()` 与 `stream()` 各执行一次（`session/service.py` `start` / `stream`），token 网关按 `(tenant, user)` 缓存凭据包（`token_gateway.py` authenticate），缓存命中时除主动续期外不再打 MES。
>
> 执行模型：交互执行与 SSE 解耦。`stream()` 里 `claim_run`（CAS，`persistence/session_store.py`）认领运行权，赢家 spawn 后台执行器（`session/lifecycle.py` `_spawn_executor` → `session/executor.py` `_execute`）跑 `_pipeline`；SSE 订阅者（含重放/历史）从事件流读取，实时/重放/历史三路逐字节一致（`api/sse.py`、`session/history.py`）。
...
正常成功路径：**3 次 LLM 调用**（均经 LiteLLM Router，`llm/router_gateway.py`；三个别名均为双部署：DeepSeek 优先级 1 为主、自建 Qwen3.8 端点优先级 2，`configs/knowledge/models.yaml`）。
...
| 业务取数 | `GongziMxQuery`（params：`scheme=""`、`Type="0,1,2"`、`Flag="0"`、`queryFooter="1"`、`dates/datee`、`Uid=本人`） | 工资明细行 | ⌈总行数/实际页大小⌉ 次 HTTP。实际页大小来自 `PagerBudget`（`data_api/pagination.py`，生产由 `bootstrap.py` 注入 `mes_page_size`/`mes_page_size_max`：**首页 20000 探测；探测暴露的总行数超过一页即一次性升至 ceiling 50000/页并从第 1 页重走**，且总页数 ≤ `max_pages=20`、总行数 ≤ `max_rows=250000`）；`KernelSettings.page_size=200` 只是 `ExecutionRequest` 上的字段，透传到 adapter 后未被 `_resource_params` 使用 |

**典型总数：3 次 LLM + 1~2 次 MES HTTP**（一个月明细行通常 <2 万，首页即可取完；token 冷启动时 +1 次 token 交换）。
...
| 9 | ④ MES 取数（分页） | **MES HTTP** | 每页 10s 超时预算；内网/真实客户网络差异大 | dates/datee/Uid/Type… → 校验后的明细行 + footer |
...
- recipe 层保留了 `parallel_group` 声明与 DAG 分组（`execution/dag.py` `_parallel_groups`），但生产 kernel 的 `_fetch_api_steps` 按 recipe 步骤顺序**逐个 await**（`kernel.py` `for step in recipe.steps`）；**且 `RecipeDagExecutor` 在生产代码中零引用（仅 `tests/unit/execution/test_dag.py` 使用），仓库内唯一声明 `parallel_group` 的 recipe 是 `smoke.yaml`**。多 API 步骤/多页 fan-out 目前是串行 HTTP。
...
- 明确的优化点（按收益排序）：
  ...
  3. recipe 的 `parallel_group` 接入 kernel 并发执行（fan-out 场景如 FR-009 每订单一调，收益随订单数线性放大）。**注意这不是「接线一步」：要复活一个生产零引用的执行器并统一其预算语义（`ExecutionBudget` vs `KernelSettings`），应作为独立重构立项。**
...
1. **usage_event 表（最完整）**——每次交互落库：`interaction_started`（入口、角色类别）/ `interaction_routed`（capability，可空 = 已路由但未命中；2026-09-19 新增）/ `llm_call_completed`（stage、model_alias、actual_model、attempt、tokens、duration_ms）/ `mes_call_completed`（operation_id、page_count、row_count_bucket、duration_ms）/ `interaction_completed`（总 duration_ms、mes_duration_ms、result_rows_bucket、error_category）。所有事件共享同一 `trace_id`/`interaction_id`（`application/usage.py` UsageContext.envelope）。
...
2. **SSE 阶段事件**——`agent_interaction_event` 表 / SSE 流中的 `interaction.phase`（阶段**完成**，`session/outcomes.py` `_phase`）与 `interaction.progress`（阶段**进行中**，同文件 `_progress`）事件自带 `stage` 与 `duration_ms`（自运行起累计）。`progress` 不推进状态机、也不写 interaction 行的生命周期列，只用于填充解析 / 权限校验 / 目录解析这三段无其他事件的等待窗口。
...
> 换句话说：**主路径在代码层面已经是「LLM ISO 优先」，规则层今天就已经是兜底**。真正缺的只有四件事：① SYSTEM_PROMPT 的 JSON 示例没展示这两个键，模型不知道该填；② prompt 未注入「今天」，模型算不出相对时间；③ `_parse_datetime` 拒绝不带时区的值（naive 直接丢弃）；④ 没有红线校验（当前只有 `start >= end`）。
...
| 单次延迟 | 输出多 ~30-60 token（两个 ISO 串），主路径为 DeepSeek 云端（云侧往返 + 生成），量级 <100ms 但比纯本地推理更贵 |
...
> **执行调整**：实际采用的路径是「直接进入阶段三终态」——LLM 主路径 + 规则层降级为兜底与红线校验 + 规则词表冻结，跳过阶段一的影子验证期，用 `FACTORY_AGENT_TIME_PARSE_MODE` 开关承载回退（详见 `docs/design/链路优化方案.md` 任务 B）。跳过影子期的补偿是把阶段一的一致性对照降级为 debug 级结构化日志 `intent.time_mismatch`（不入库、不影响行为）。
...
api/sessions.py (start_interaction / stream_interaction)
  ...
  └─ application/session/（包） SessionService.start / stream / _pipeline
       ...
       ├─ export_service.py + export/xlsx.py（内存渲染 XLSX，落盘保留）
       ├─ ports/card.py build_card（结果卡片：kind ∈ kpi/table/ranking，
       │    形态由 recipe `card:` 块声明，加载期 execution/recipes.py 校验 fail fast）
  ...
  └─ api/sse.py（SSE 实时推送；实时/重放/历史三路一致）
       └─ application/session/history.py（历史事件重放 → 多轮上下文）
```
...
## 任务五：逐环节排查索引（用户 query → 最终回答）

> 用途：逐环节检查/排障时，按本表从上到下定位「主看文件」。每行给：该环节要回答的问题 → 主看文件（关键入口）→ 辅助文件。全部路径相对 `src/factory_agent/`。

### 5.1 入口与身份（环节 0~2）

| # | 环节 | 要确认的问题 | 主看文件（入口函数/类） | 辅助文件 |
|---|---|---|---|---|
| 0 | HTTP 路由 | 请求进来落在哪个 handler？request_id 怎么生成？ | `api/server.py`（`create_app`）、`api/sessions.py`（`start_interaction` / `stream_interaction`） | `api/sse.py`（SSE 推送实现） |
| 1 | 身份解析 | 加密 app_key 如何换取凭据？角色/部门从哪来？ | `api/identity.py`（`resolve_credential`）→ `data_api/token_gateway.py`（`TokenCredentialExchange`、`_principal_from_bundle`） | `data_api/credentials.py`（凭据束类型） |
| 2 | MES token 交换 | 何时打 `/api/system/token`？缓存/续期策略？ | `data_api/token_gateway.py`（authenticate、续期阈值在 `config.py`） | — |

### 5.2 会话与运行权（环节 3~4）

| # | 环节 | 要确认的问题 | 主看文件（入口函数/类） | 辅助文件 |
|---|---|---|---|---|
| 3 | 创建 interaction + 鉴权 | `authorize` 何时执行？interaction/消息何时落库？ | `application/session/service.py`（`SessionService.start` / `stream` / `cancel`）；`application/authorization.py`（`AuthorizationService`） | `application/session/base.py`（`_commit`、`_advance`）、`persistence/session_store.py` |
| 4 | 运行权认领与后台执行 | 防重复执行的 CAS 在哪？后台执行器如何 spawn？僵尸 run 怎么清理？ | `persistence/session_store.py`（`claim_run`）、`application/session/lifecycle.py`（`_spawn_executor`、`fail_stale_run`、`sweep_stale_runs`） | `application/session/executor.py`（`_execute`、取消中断）、`ports/session.py` |

### 5.3 解析与鉴权分支（环节 5~9，主状态机）

> 主状态机 = `application/session/pipeline.py::_pipeline`；分支函数分布在 mixin：`_parse`/`_chat`/`_compose_result_answer` 在 pipeline.py，`_clarify`/`_reject`/`_fail`/`_terminate`/`_phase` 在 outcomes.py，`_run_scope_guard` 在 consistency.py。怀疑「流程顺序不对」时只看 `_pipeline` 一个函数即可。

| # | 环节 | 要确认的问题 | 主看文件（入口函数/类） | 辅助文件 |
|---|---|---|---|---|
| 5 | LLM 意图解析（EXTRACT） | 能力选择/槽位/闲聊分类/多轮改写怎么做？JSON 失败怎么补？ | `application/intent.py`（`SYSTEM_PROMPT`、`CapabilityIntentParser.parse`、`_build_slots`） | `application/structured.py`（REPAIR）、`llm/router_gateway.py`（Router）、`configs/knowledge/models.yaml` |
| 6 | 澄清/闲聊分支 | 什么时候追问？最多几轮？闲聊回复从哪来？ | `application/session/outcomes.py`（`_clarify`、`_terminate`）、`application/session/definitions.py`（轮数上限） | `application/chitchat.py`（CHAT 兜底）、`application/session/history.py`（多轮上下文） |
| 7 | 权限矩阵 | (capability, role) 的 allow/deny 表？拒绝文案？ | `application/permission_matrix.py`（`authorize_capability`、`CAPABILITY_ROLES`、`ROLE_DATA_RANGE`） | `application/capability_map.py`（FR↔recipe 映射） |
| 8 | 越权护栏（SCOPE_GUARD） | LLM 判越权的 prompt 与 fail-open 语义？拦截审计在哪？ | `application/scope_guard.py`（`ScopeGuard.check`）、`application/session/consistency.py`（`_run_scope_guard`、`_record_scope_violation`） | `observability/audit.py`、`application/scope_review.py`、`persistence/scope_violation.py` |
| 9 | 业务过滤与收窄 | 部门/员工名槽位怎么解析成 ID？个人能力怎么强制绑本人？ | `application/business_filters.py`（`BusinessFilterResolver`）、`application/filters.py`（`FilterNarrower` → `NarrowedFilters`） | `data_api/directory.py`（MES EmployeeQuery/DeptQuery）、`application/cache.py`（Redis 缓存）、`application/time_expressions.py`（时间规则） |

### 5.4 取数与计算（环节 10~13）

| # | 环节 | 要确认的问题 | 主看文件（入口函数/类） | 辅助文件 |
|---|---|---|---|---|
| 10 | 能力执行编排 | API 步骤顺序、翻页预算、沙箱注册在哪控制？ | `execution/kernel.py`（`KernelCapabilityRunner.run`、`_fetch_api_steps`、`KernelSettings`） | `execution/recipes.py`（DAG 加载校验）、`execution/dag.py`（并行分组，未接入并发） |
| 11 | MES 调用与范围强制 | 白名单、参数来源、pydantic 校验、有界翻页？ | `execution/executor.py`（`ScopedExecutor`）→ `data_api/hongzhao.py`（`HongzhaoMesAdapter`，**全仓唯一允许出现 MES URL/凭据/客户字段名的地方**） | `data_api/catalog.py`（apis.yaml 加载）、`data_api/schemas.py`、`data_api/pagination.py`（BoundedPager）、`configs/knowledge/apis.yaml` |
| 12 | 本地计算与口径 | 只读 SQL 怎么跑？指标口径/footer 对账在哪？ | `execution/sandbox.py` / `sandbox_runtime.py`（DuckDB）、`execution/result_table.py`（指标注册表 + ResultTable） | `configs/knowledge/recipes/*.yaml`（SQL、口径版本、footer） |
| 13 | 一致性安全网 | 结果行范围 vs 角色期望范围，exact 拦截/heuristic 告警？ | `application/consistency.py`（`ConsistencyValidator.validate`），模式开关 `FACTORY_AGENT_VALIDATION_MODE` | `application/session/consistency.py`（拦截文案与审计） |

### 5.5 回答与事件（环节 14~16）

| # | 环节 | 要确认的问题 | 主看文件（入口函数/类） | 辅助文件 |
|---|---|---|---|---|
| 14 | 答案组织（SUMMARIZE） | prompt 喂了什么？失败兜底？ | `application/summary.py`（`ResultSummarizer.summarize`、`RESULT_ANSWER_SYSTEM_PROMPT`）；调用点 `application/session/pipeline.py::_compose_result_answer` | — |
| 15 | 结果卡片与导出 | card 形态怎么声明/构建？XLSX 如何留存与清理？ | `ports/card.py`（`CardTableSpec`、`build_card`）、`export_service.py` + `export/xlsx.py`（内存渲染后落盘保留，默认 90 天）；card 挂载点 `pipeline.py::_complete_result` | `configs/knowledge/recipes/*.yaml`（`card:` 块）、`execution/recipes.py`（`_validate_card`、`to_table_spec`） |
| 16 | SSE 事件与终态 | result/card 事件怎么发？三路（实时/重放/历史）一致性靠什么？ | `application/session/pipeline.py`（`_complete_result`，card 同时进 result_table 消息 payload）、`api/sse.py` | `application/session/history.py`（重放）、`application/session/outcomes.py`（`_phase`/`_terminate`）、`domain/session.py`（状态机定义） |
| 17 | 持久化与计量 | 业务事件与用量事件分别怎么落库？失败会不会拖垮业务？ | `persistence/session_store.py`（`SqlInteractionStore`）、`application/usage.py`（事件构造）、`persistence/metering.py`（独立事务直写） | `persistence/tables.py`、`persistence/engine.py` |

### 5.6 全局配置（横切所有环节）

| 想改/想查 | 文件 |
|---|---|
| 全部 env 配置项（LLM/会话/超时/时区/校验模式） | `config.py`（`FactoryAgentSettings`）、`.env.example` |
| 模型别名 → 部署 → fallback | `configs/knowledge/models.yaml`、`llm/registry.py` |
| 依赖组装与组件接线 | `bootstrap.py`（`build_container`、`_build_session_service`、`_build_capability_runner`） |
| 日志字段与脱敏 | `observability/logging_adapter.py`、`observability/redaction.py` |
| 排障取数（耗时/事件时间线） | 见 2.5 节（usage_event SQL / SSE 阶段事件 / 结构化日志） |
