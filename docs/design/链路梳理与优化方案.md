...
flowchart TD
    ...
    T --> U["按 recipe DAG 执行 API 步骤（串行）<br/>ScopedExecutor.execute_full_step → HongzhaoMesAdapter<br/>④ MES HTTP：GongziMxQuery（分页由 PagerBudget 驱动：首页 2000，必要时升至 ceiling 50000）<br/>pydantic 校验 → 行写入 DuckDB 沙箱"]
...
| 业务取数 | `GongziMxQuery`（params：`scheme=""`、`Type="0,1,2"`、`Flag="0"`、`queryFooter="1"`、`dates/datee`、`Uid=本人`） | 工资明细行 | ⌈总行数/实际页大小⌉ 次 HTTP。实际页大小来自 `PagerBudget`（`data_api/pagination.py`，生产由 `bootstrap.py` 注入 `mes_page_size`/`mes_page_size_max`：**首页 2000，必要时升至 ceiling 50000**，且总页数 ≤ `max_pages=20`、总行数 ≤ `max_rows=250000`）；`KernelSettings.page_size=200` 只是 `ExecutionRequest` 上的字段，透传到 adapter 后未被 `_resource_params` 使用 |

**典型总数：3 次 LLM + 1~2 次 MES HTTP**（一个月明细行通常 <2000，首页即可取完；token 冷启动时 +1 次 token 交换）。
