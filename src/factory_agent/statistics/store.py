...
PostgreSQL implementation reads the metering tables (``usage_event`` /
``*_fact`` / ``tenant_usage_*``) and owns the writes to the statistics
surface's own tables (``tenant_registry`` / ``platform_principal`` /
``admin_audit`` / ``usage_export``).

Every statement runs on a short-lived connection of its own, opened with the
statistics session profile: read paths additionally default to read-only
transactions, and all paths carry ``statement_timeout`` so a platform-wide
aggregate can never pin a connection indefinitely. The statistics surface
never shares a pool with the session pipeline.
"""
...
from factory_agent.statistics.events import InteractionFact, LlmCallFact, MesCallFact
...
class TenantRegistryRecord:
    ...
    ``app_key`` is the primary key and the internal tenant identifier (D12);
    ``status`` is ``active`` or ``disabled`` (D10/D13). AppKey is stored in
    plaintext (D9) but every outbound response masks it. ``tenant_ref`` is the
    non-secret handle every operator-facing operation addresses: an
    automatically registered tenant has no create response that could have
    shown its plaintext AppKey, and a masked AppKey collides across tenants
    that share a prefix, so audit would not be traceable without it.
    """
...
class MesOperationCategory:
    """operation_id -> billing category mapping."""
...
class UsageStore(Protocol):
    ...
    async def get_tenant_registry_by_ref(self, tenant_ref: str) -> TenantRegistryRecord | None: ...

    async def register_seen_tenant(
        self,
        *,
        app_key: str,
        tenant_ref: str,
...
class InMemoryUsageStore:
    ...
    async def register_seen_tenant(
        ...
        *,
        ...
        if app_key in self.tenant_registry:
            return False
        self.tenant_registry[app_key] = TenantRegistryRecord(
            app_key=app_key,
...
async def _connect(
    ...
    *,
    ...
    """Open one short-lived connection with the statistics session profile.

    The statistics surface never shares the session pipeline's engine or pool; a
    platform-wide aggregate must not be able to queue behind (or ahead of)
    business traffic. ``options`` is a libpq startup parameter, so the timeout
    and the read-only default are in force from the very first transaction
    instead of being applied after it.
    """
    options: list[str] = []
    ...
    kwargs: dict[str, Any] = {"row_factory": psycopg.rows.dict_row}
    if options:
        kwargs["options"] = " ".join(options)
    return await psycopg.AsyncConnection.connect(database_url, **kwargs)
...
class PostgresUsageStore:
    ...
    parameterised. Reads cover the metering and rollup tables and write
    connections touch only this surface's own tables (``tenant_registry`` /
    ...
    async def _read(self) -> psycopg.AsyncConnection[Any]:
    ...
    async def _write(self) -> psycopg.AsyncConnection[Any]:
        return await _connect(
            self._database_url,
            statement_timeout_ms=self._statement_timeout_ms,
        )
    ...
    async def list_interaction_facts(
        ...
        if not tenant_ids:
            return []
        async with await self._read() as connection:
    ...
    async def record_audit(self, entry: AuditEntry) -> None:
        async with await self._write() as connection:
    ...
    async def get_tenant_registry_by_ref(self, tenant_ref: str) -> TenantRegistryRecord | None:
        async with await self._read() as connection:
            rows = await connection.execute(
                "SELECT * FROM tenant_registry WHERE tenant_ref = %s", (tenant_ref,)
            )
            row = await rows.fetchone()
        if row is None:
            return None
    ...
    async def register_seen_tenant(
        ...
        *,
        ...
        """Idempotent first-seen insert; ``True`` when this call created the row."""
        async with await self._write() as connection:
            rows = await connection.execute(
                """
                INSERT INTO tenant_registry
                    (app_key, tenant_ref, tenant_name, status, created_at, updated_at)
                VALUES (%s, %s, %s, 'active', %s, %s)
                ON CONFLICT (app_key) DO NOTHING
                RETURNING app_key
                """,
                (app_key, tenant_ref, tenant_name, at, at),
            )
            row = await rows.fetchone()
