...
``mes_call_fact``) are written inside the business transaction; the statistics
surface reads them read-only for platform reporting. The dataclasses here
mirror one row of each table.

Payload validation happens on the writer side before a row is stored; there is
no ingest path in the statistics surface.
"""
...
@dataclass(frozen=True, slots=True)
class MesCallFact:
    """One customer MES request per row (read-only for the statistics surface).
