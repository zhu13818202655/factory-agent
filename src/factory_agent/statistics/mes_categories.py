...
by API business category — not by agent capability. The authoritative source of
the mapping is ``mes_operation_category`` (seeded from
``configs/knowledge/apis.yaml``); this module also carries the reviewed default
mapping so the surface behaves correctly when that table is not yet migrated,
and so tests can run offline.
...
from factory_agent.statistics.store import MesOperationCategory, UsageStore
...
class MesCategoryResolver:
    ...
    The live mapping comes from the ``mes_operation_category`` table; when the
    table is not present yet or a row is missing, the reviewed default mapping
