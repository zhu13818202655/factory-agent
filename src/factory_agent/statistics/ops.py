...
from factory_agent.statistics.mes_categories import (
    CATEGORY_ORDERING,
    CATEGORY_OTHER,
    MesCategoryResolver,
)
from factory_agent.statistics.platform import PlatformScope, PlatformScopeError
from factory_agent.statistics.store import TenantRegistryRecord, UsageStore, hour_bucket

Granularity = Literal["hour", "day"]

#: Rollup rows are produced by factory-agent (``rollup-v3``); this
...
class OpsLimits:
    ...
    #: How far the rollup may fall behind the newest raw fact before a response
    #: calls itself incomplete. The composition root raises it to twice the
    #: configured sweep interval so a healthy worker never trips it between
    #: passes; the default stands on its own when nothing is injected.
    rollup_lag_tolerance: timedelta = timedelta(minutes=10)
...
class TenantOption:
    ...
    Deliberately carries no AppKey: the plaintext key is a secret (D9), so the
    selector is addressed by the non-secret ``tenant_ref`` instead.
    """
...
class OpsService:
    ...
    async def list_tenants(
        ...
        """Factories with data in the window, addressed by their non-secret handle.

        The dashboard's factory selector used to receive plaintext AppKeys, which
        handed every reader of the usage board a credential it has no use for.
        It now receives ``tenant_ref`` + name; a tenant that has no registry row
        (hence no handle to address it by) is omitted rather than described by
        its key.
        """
        self._check_range(start, end)
        tenants = await self._store.list_tenants(start, end)
        visible = [tenant for tenant in tenants if scope.covers_tenant(tenant)]
        if not visible:
            return []
        by_key = {record.app_key: record for record in await self._store.list_all_tenant_registry()}
        options: list[TenantOption] = []
        for tenant in visible:
            record = by_key.get(tenant)
            if record is None:
                continue
            options.append(TenantOption(record.tenant_ref, record.tenant_name))
        return options
    ...
    async def _tenant_ids(
        ...
        """Resolve the effective tenant set, optionally narrowed to one factory.

        ``tenant_ref`` is the dashboard's factory filter. A handle outside the
        caller's scope is refused (a filter must never become a probe for tenants
        the token cannot see), and a handle whose factory simply has no data in
        the window narrows the result to the empty set — that is a real answer,
        not an error.
        """
        if scope.tenant_ids:
            base = scope.tenant_ids
        else:
            base = frozenset(await self._store.list_tenants(start, end))
        if tenant_ref is None:
            return base
        _, by_ref = await self._registry_index()
        return frozenset({self._resolve_ref(scope, by_ref, tenant_ref)}) & base

    async def _registry_index(
        self,
    ) -> tuple[dict[str, TenantRegistryRecord], dict[str, str]]:
        """Both views of the registry: by AppKey (rows) and by handle (AppKeys)."""
        records = await self._store.list_all_tenant_registry()
        return (
            {record.app_key: record for record in records},
            {record.tenant_ref: record.app_key for record in records},
        )

    def _resolve_ref(self, scope: PlatformScope, by_ref: dict[str, str], tenant_ref: str) -> str:
        """Resolve a non-secret handle to its AppKey, refusing an unusable one."""
        app_key = by_ref.get(tenant_ref)
        if app_key is None:
            raise OpsQueryError(f"unknown tenant_ref: {tenant_ref}")
        if not scope.covers_tenant(app_key):
            raise PlatformScopeError("requested tenant set exceeds the platform scope")
        return app_key

    async def summary(
        self, scope: PlatformScope, start: datetime, end: datetime, *, tenant_ref: str | None = None
    ) -> SummaryView:
        self._check_range(start, end)
        tenant_ids = await self._tenant_ids(scope, start, end, tenant_ref)
        if not tenant_ids:
        ...
        incomplete = _rollup_behind(rollup, freshness, self._limits.rollup_lag_tolerance)
        return SummaryView(
    ...
    async def timeseries(
        ...
        *,
        ...
        return TimeseriesView(
            ...
            metric_version=self._metric_version(),
            timezone=self._timezone_name,
            # Two independent reasons a chart is not the whole story: the row cap
            # dropped buckets, or the rollup itself has not caught up with the raw
            # facts in this window.
            incomplete=truncated
            or _rollup_behind(rows, freshness, self._limits.rollup_lag_tolerance),
        )
    ...
    async def capabilities(
        ...
        *,
        ...
        return await self.dimensions(scope, start, end, "capability", tenant_ref=tenant_ref)
    ...
    async def by_tenant(
        ...
        *,
        ...
        The exact-factory filter takes a ``tenant_ref``, not an AppKey: accepting
        a plaintext key as a query parameter is what forced the selector endpoint
        to hand keys out in the first place.
        """
        self._check_range(start, end)
        if offset < 0:
            raise OpsQueryError("offset must not be negative")
        page_size = self._clamp_page(limit)
        by_key, by_ref = await self._registry_index()

        requested: set[str] = set()
        if tenant_ref:
            requested.add(self._resolve_ref(scope, by_ref, tenant_ref))
        if name:
...
def _rollup_behind(rows: list[Any], freshness: datetime | None, tolerance: timedelta) -> bool:
    """Has the rollup fallen behind the raw facts it is supposed to summarise?

    The comparison uses the two clocks the pipeline already keeps: ``freshness``
    is the newest raw fact written in the window, and ``rolled_up_at`` is when a
    bucket was last recomputed. A live worker re-stamps every window it covers,
    so steady state satisfies ``rolled_up_at >= freshness``; a stopped one
    freezes while facts keep arriving, which flips this to ``True`` — and, unlike
    a per-bucket coverage check, clears itself as soon as the worker catches up.

    ``tolerance`` absorbs the normal "a fact just landed, this cycle has not run
    yet" gap, so a healthy worker does not flicker.

    No facts in the window means there is nothing for the rollup to be behind on:
    an empty window is "no usage", not "missing aggregation".
    """
    ...
    if not rows:
        return True
    stamps = [
        stamp for row in rows if isinstance(stamp := getattr(row, "rolled_up_at", None), datetime)
    ]
    if not stamps:
        return True
    return max(stamps) < freshness - tolerance
