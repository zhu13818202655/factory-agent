"""Fault injection tests adapted to the customer envelope (Story 5)."""
...
async def test_structural_fault_is_request_scoped(fault: str) -> None:
    ...
    if fault == "duplicate_page":
        assert len(payload["list"]) == payload["total"] + 1
        ...
        # Footer disagrees with the visible rows: drift detection case.
