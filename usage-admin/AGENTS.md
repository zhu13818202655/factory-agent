...
- **Table ownership (breaking change, Story 9 / product doc §4.4)**: this service owns and writes
  `tenant_registry`, `admin_audit`, and `platform_principal`; every other table in the shared
  PostgreSQL (business tables and all metering tables, including `mes_call_fact` and
  `mes_operation_category`) is factory-agent owned and **read-only here**. No table may appear in
  both migration histories; this service's Alembic directory contains only its three tables.
- **Separate Alembic version table**: `alembic_version_usage_admin` (factory-agent uses
  `alembic_version_factory_agent`) so both services can migrate one shared database in any order.
