...
from factory_agent.statistics.ops import OpsQueryError, OpsService
...
async def test_by_tenant_returns_registry_names_statuses_and_counts() -> None:
    store = build_store()
    from factory_agent.statistics.store import RollupRow
...
@pytest.mark.asyncio
async def test_by_tenant_name_and_tenant_ref_filters() -> None:
    store = build_store()
    service = ops(store)
    ...
    by_ref = await service.by_tenant(
        VIEWER, START, END, limit=10, offset=0, tenant_ref="t_0e0f1011"
    )
    assert [item.app_key for item in by_ref.items] == ["fac-02"]


@pytest.mark.asyncio
async def test_by_tenant_rejects_unknown_tenant_ref() -> None:
    service = ops(build_store())

    with pytest.raises(OpsQueryError, match="unknown tenant_ref"):
        await service.by_tenant(VIEWER, START, END, limit=10, offset=0, tenant_ref="t_deadbeef")


@pytest.mark.asyncio
async def test_list_tenants_returns_handles_and_names_never_keys() -> None:
    service = ops(build_store())

    options = await service.list_tenants(VIEWER, START, END)

    assert [(option.tenant_ref, option.tenant_name) for option in options] == [
        ("t_0a0b0c0d", "温州一厂"),
        ("t_0e0f1011", "杭州二厂"),
    ]
    assert not any("fac-" in option.tenant_ref for option in options)
...
async def test_by_tenant_rejects_out_of_scope_filter() -> None:
    ...
    with pytest.raises(PlatformScopeError, match="exceeds"):
        await service.by_tenant(SCOPED, START, END, limit=10, offset=0, tenant_ref="t_0e0f1011")
...
async def test_models_aggregate_calls_and_tokens() -> None:
    store = InMemoryUsageStore()
    from tests.statistics.support.events import llm_call_completed
...
async def test_errors_break_down_across_fact_types() -> None:
    store = InMemoryUsageStore()
    from tests.statistics.support.events import interaction_completed, llm_call_completed
...
# Every aggregate endpoint funnels its tenant set through OpsService._tenant_ids,
# so the factory filter has to mean the same thing everywhere. The name list below
# is what makes that a standing guarantee rather than a by-tenant special case.
AGGREGATE_METHODS = (
    "summary",
    "timeseries",
    "dimensions",
...
async def _call_aggregate(service: OpsService, name: str, scope: PlatformScope, **kwargs: object):
    method = getattr(service, name)
    if name == "timeseries":
        return await method(scope, START, END, "day", ("questions",), **kwargs)
    if name == "dimensions":
        return await method(scope, START, END, "capability", **kwargs)
    if name == "users":
        return await method(scope, START, END, None, **kwargs)
    return await method(scope, START, END, **kwargs)


@pytest.mark.asyncio
@pytest.mark.parametrize("name", AGGREGATE_METHODS)
async def test_every_aggregate_honours_the_factory_filter(name: str) -> None:
    service = ops(build_store())

    everything = await _call_aggregate(service, name, VIEWER)
    assert set(everything.tenant_ids) > {"fac-02"}

    narrowed = await _call_aggregate(service, name, VIEWER, tenant_ref="t_0e0f1011")
    assert narrowed.tenant_ids == ("fac-02",)


@pytest.mark.asyncio
@pytest.mark.parametrize("name", AGGREGATE_METHODS)
async def test_every_aggregate_rejects_an_unknown_factory_handle(name: str) -> None:
    service = ops(build_store())

    with pytest.raises(OpsQueryError, match="unknown tenant_ref"):
        await _call_aggregate(service, name, VIEWER, tenant_ref="t_deadbeef")


@pytest.mark.asyncio
@pytest.mark.parametrize("name", AGGREGATE_METHODS)
async def test_every_aggregate_refuses_an_out_of_scope_factory(name: str) -> None:
    """A filter must never become a probe for tenants the token cannot see."""
    service = ops(build_store())

    with pytest.raises(PlatformScopeError, match="exceeds"):
        await _call_aggregate(service, name, SCOPED, tenant_ref="t_0e0f1011")
