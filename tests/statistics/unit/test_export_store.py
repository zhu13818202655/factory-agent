...
from factory_agent.statistics.exports import ExportService, sign_download
...
def test_backend_selection_prefers_s3_then_local_then_memory() -> None:
    s3 = build_export_file_store(
        StatisticsSettings(
            ...
            s3_bucket="factory-agent-statistics-exports",
    ...
    local = build_export_file_store(StatisticsSettings(export_store_dir="/tmp/ua-exports"))
    assert isinstance(local, LocalExportFileStore)

    # S3 wins when both are configured.
    both = build_export_file_store(
        StatisticsSettings(s3_endpoint_url="http://seaweedfs:8333", export_store_dir="/tmp/ua")
    )
    assert isinstance(both, S3ExportFileStore)

    assert isinstance(build_export_file_store(StatisticsSettings()), InMemoryExportFileStore)


def test_container_uses_the_configured_backend(tmp_path: Path) -> None:
    container = build_container(StatisticsSettings(export_store_dir=str(tmp_path / "exports")))
...
async def test_a_restart_still_serves_the_signed_download(tmp_path: Path) -> None:
    ...
    def service() -> ExportService:
        return ExportService(
            ...
            download_base_url="http://statistics.test",
        )
