...
from factory_agent.statistics.platform import PlatformRole, PlatformScope
from factory_agent.statistics.store import InMemoryUsageStore, RollupRow
from tests.statistics.support.events import (
...
async def build_store() -> InMemoryUsageStore:
    ...
    (rollup-v2); the statistics surface only reads them.
    """
...
# --- incomplete 的四象限：原始事实 vs 汇总（rollup-v3 语义）----------------------
#
# 判据是「窗口内已有原始事实，但汇总的 rolled_up_at 落后于它」；窗口内没有原始事实
# 时是「这段期间没有用量」，不是「汇总缺失」，不能报 incomplete。


@pytest.mark.asyncio
async def test_a_rollup_that_fell_behind_the_facts_is_incomplete() -> None:
    store = await build_store()
    stale = NOW - timedelta(hours=1)
    store.rollup_rows = [replace(row, rolled_up_at=stale) for row in store.rollup_rows]

    view = await ops(store).summary(VIEWER, START, END)

    assert view.incomplete is True
    # The figure is still served — it is flagged, not withheld.
    assert view.questions == 2


@pytest.mark.asyncio
async def test_facts_without_any_rollup_row_are_incomplete() -> None:
    store = await build_store()
    store.rollup_rows = []
...
@pytest.mark.asyncio
async def test_a_caught_up_rollup_is_not_incomplete() -> None:
    service = ops(await build_store())
...
@pytest.mark.asyncio
async def test_an_empty_window_is_no_usage_not_missing_aggregation() -> None:
    view = await ops(InMemoryUsageStore()).summary(VIEWER, START, END)
...
@pytest.mark.asyncio
async def test_rollup_rows_without_facts_in_the_window_are_not_incomplete() -> None:
    """The facts aged out of the window; nothing is behind any more."""
    store = await build_store()
    store.interaction_facts = []
    store.llm_call_facts = []

    view = await ops(store).summary(SCOPED, START, END)
...
@pytest.mark.asyncio
async def test_timeseries_incomplete_follows_the_same_rule() -> None:
    behind = await build_store()
    stale = NOW - timedelta(hours=1)
    behind.rollup_rows = [replace(row, rolled_up_at=stale) for row in behind.rollup_rows]
    caught_up = await build_store()

    behind_view = await ops(behind).timeseries(VIEWER, START, END, "hour", ("questions",))
    caught_up_view = await ops(caught_up).timeseries(VIEWER, START, END, "hour", ("questions",))

    assert behind_view.incomplete is True
    assert caught_up_view.incomplete is False
