"""The statistics store against a real PostgreSQL.

Set ``FACTORY_AGENT_TEST_POSTGRES_URL`` to a disposable database to enable the
suite; it applies ``migrations`` (the single baseline) and is skipped otherwise.
Metering rows are written in a separate transaction after the business commit,
so this suite seeds ``interaction_fact`` / ``llm_call_fact`` /
``tenant_usage_hourly`` / ``tenant_usage_daily`` directly with SQL (mirroring
what the business pipeline persists) and asserts the read-only queries the ops
layer relies on return correct aggregates — including the regression that hourly
rollup rows never leak across to the daily projection.
"""
...
from factory_agent.statistics.events import InteractionFact, LlmCallFact
from factory_agent.statistics.ops import OpsService
...
def _applied_migrations() -> None:  # pyright: ignore[reportUnusedFunction]
    """Apply the single baseline so the metering and registry tables exist."""
...
async def _seed_metering_rows(database_url: str) -> None:
    """Write the metering rows the business pipeline would have persisted."""
