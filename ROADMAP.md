...
- 一共 12 个 Story，按 `#1.md` 到 `#12.md` 的编号顺序推进。
...
Story 9 交付 usage-admin 用量看板与租户主数据（用量查询接口、工厂账户管理，插入项）；
Story 10 把 Mock MES 升级为 PG 持久化 + 每日定时生产化数据生成的数据底座（原 Story 9）；
Story 11 完成 factory-agent 计量直写改造（移除 outbox、同库直写、MES 接口调用统计、rollup，
插入项）；Story 12（原 Story 10）接入客户真实环境并上线。权限、租户隔离和敏感数据规则
不得留空或自行假设。
...
9. `#9.md`：usage-admin 用量看板与租户主数据（插入项）。
10. `#10.md`：Mock MES 生产化数据生成与 PostgreSQL 落地（原 `#9.md`，因插入新 Story 9
    顺延重编号）。
11. `#11.md`：factory-agent 计量直写改造（插入项；承载共库表归属与方案二在本服务侧的实施）。
12. `#12.md`：客户环境联调与生产上线（原 `#10.md`，因插入新 Story 9、11 顺延重编号）。
